...
namespace CTCBeamDecoder.CTCBeamDecoderInterop {
    ...
    internal static class CTCDecoderInterop {
        ...
        public static unsafe IntPtr GetScorer(double alpha,
            double beta,
            string lmPath,
            string[] labels) => new IntPtr(paddle_get_scorer(alpha, beta, lmPath, labels, (uint)labels.Length));
        ...
        public static unsafe IntPtr GetDecoderState(string[] labels, //num_labels
            ...
            uint blankId,
            int logInput,
            IntPtr scorer) => new IntPtr(paddle_get_decoder_state(labels, (uint)labels.Length,
            beamSize, cutoffProb,
            cutoffTopN, blankId,
        ...
        public static unsafe void ReleaseScorer(IntPtr scorer) => paddle_release_scorer(scorer.ToPointer());

        public static unsafe void ReleaseState(IntPtr scorer) => paddle_release_state(scorer.ToPointer());

        public static unsafe double GetLogCondProb(IntPtr scorer, string[] words) => get_log_cond_prob(scorer.ToPointer(), words, (uint)words.Length);

        public static unsafe double GetSentLogProb(IntPtr scorer, string[] words) => get_sent_log_prob(scorer.ToPointer(), words, (uint)words.Length);

        public static unsafe int IsCharacterBased(IntPtr scorer) => is_character_based(scorer.ToPointer());

        public static unsafe uint GetMaxOrder(IntPtr scorer) => get_max_order(scorer.ToPointer());

        public static unsafe uint GetDictionarySize(IntPtr scorer) => get_dict_size(scorer.ToPointer());

        public static unsafe void ResetParameters(IntPtr scorer, double alpha, double beta) => reset_params(scorer.ToPointer(), alpha, beta);
        ...
        private static extern unsafe int paddle_beam_decode(
            ...
            [MarshalAs(UnmanagedType.LPArray, ArraySubType = UnmanagedType.LPStr)] string[] labels, //num_labels
            uint batch_size,
            uint max_time,
            uint num_labels,
            uint beam_size,
        ...
        private static extern unsafe void * paddle_get_scorer(double alpha,
            double beta, [MarshalAs(UnmanagedType.LPStr)] string lm_path, [MarshalAs(UnmanagedType.LPArray, ArraySubType = UnmanagedType.LPStr)] string[] labels, //num_labels
            uint labels_size);
